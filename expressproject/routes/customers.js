...
router.get('/',authentication_mdl.is_login, function(req, res, next) {
	req.getConnection(function(err,connection){
		var query = connection.query('SELECT * FROM customer',function(err,rows)
		{
			...
			res.render('customer/list',{title:"Data Pasien",data:rows,session_store:req.session});
		});
		...
		req.getConnection(function(err,connection){
			var query = connection.query(delete_sql, customer, function(err, result){
				if(err)
				{
					...
					req.flash('msg_info', 'Berhasil Menghapus Data Pasien'); 
					res.redirect('/customers');
				}
			});
		});
		...
		req.getConnection(function(err,connection){
			var query = connection.query(update_sql, customer, function(err, result){
				if(err)
				{
					...
					req.flash('msg_info', 'Berhasil Mengupdate Data Pasien'); 
		...
		req.getConnection(function(err,connection){
			var query = connection.query(insert_sql, customer, function(err, result){
				if(err)
				{
					...
					req.flash('msg_info', 'Berhasil Menambah Data Pasien'); 
					res.redirect('/customers');
				}		
			});
		});
